...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    ...
    if(current_servo == 0){
    	...
    	for(int i = 0; i < num_active_servos; i++){
    		HAL_GPIO_WritePin(servos_sorted[i].servo_port, servos_sorted[i].servo_pin, GPIO_PIN_SET);
    	}
		__HAL_TIM_SET_COUNTER(htim_used, 0);
		__HAL_TIM_SET_COMPARE(htim_used, TIM_CHANNEL_1, servos_sorted[current_servo].servo_microseconds);
    	current_servo += 1;
    }
    else if(current_servo < num_active_servos){
    	HAL_GPIO_WritePin(servos_sorted[current_servo - 1].servo_port, servos_sorted[current_servo - 1].servo_pin, GPIO_PIN_RESET);
		__HAL_TIM_SET_COMPARE(htim_used, TIM_CHANNEL_1, servos_sorted[current_servo].servo_microseconds);
		current_servo += 1;
    }
    else{
    	HAL_GPIO_WritePin(servos_sorted[current_servo].servo_port, servos_sorted[current_servo].servo_pin, GPIO_PIN_RESET);
    	current_servo = 0;
		__HAL_TIM_SET_COMPARE(htim_used, TIM_CHANNEL_1, 20000);
    }
}

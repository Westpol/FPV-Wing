...
DMA_HandleTypeDef *gps_dma;
uint32_t buffer_overflow_count = 0;
uint8_t dma_buffer[GPS_BUFFER_SIZE] = {0};

PARSE_STRUCT parse_struct = {0};
...
void GPS_DUMP(){
	USB_PRINTLN_RAW(dma_buffer, GPS_BUFFER_SIZE);
}
...
void GPS_PARSE_BUFFER(void){
	...
	if(dma_pos > parse_struct.parser_position && buffer_overflow_count == parse_struct.parser_overflow_count){		// if the buffer has not wrapped around

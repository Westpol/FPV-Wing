...
static void MX_GPIO_Init(void)
{
  ...
  /**/
  LL_GPIO_SetOutputPin(CS_EXT_1_GPIO_Port, CS_EXT_1_Pin);
  ...
  /**/
  GPIO_InitStruct.Pin = CS_EXT_1_Pin;
  GPIO_InitStruct.Mode = LL_GPIO_MODE_OUTPUT;
  GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_LOW;
  GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
  GPIO_InitStruct.Pull = LL_GPIO_PULL_UP;
  LL_GPIO_Init(CS_EXT_1_GPIO_Port, &GPIO_InitStruct);
...
static void PRINT_DATA(){
	...
	//USB_PRINTLN("Pitch angle: %f  |  Roll angle: %f  |  CPU usage avrg 1s: %.1f%%  |  CPU max usage 1s: %.1f%%  |  Time: %ld", ONBOARD_SENSORS.gyro.pitch_angle, ONBOARD_SENSORS.gyro.roll_angle, USAGE_STAT_GET_AVRG_1S() / 10.0f, USAGE_STAT_GET_MAX_LOOP_TIME_1S() / 10.0f, MICROS64());
	USB_PRINTLN("%f, %f", attitude_pid.pitch_error * (180.0f / M_PI), attitude_pid.roll_error * (180.0f / M_PI));
